...
{
   auto jet = std::make_pair(jet_1.first.m_lvec.Pt(), jet_1.second);
   float sf1 = CalcCCTagWeight_1jet(jet, jet_1.first.m_flav, uncType);
   jet = std::make_pair(jet_2.first.m_lvec.Pt(), jet_2.second);
   float sf2 = CalcCCTagWeight_1jet(jet, jet_2.first.m_flav, uncType);
   return sf1*sf2;
}

float Selector::CalcCCTagWeight_1jet(std::pair<float,bool> jet, int jet_flav, std::string uncType)
{
  float sf = 1.0f;
  float eff = 1.0f;
  float sf_tmp = 1.0f;
  
  std::string tagType("xcc");
  // Get the efficiency. 
  int iB = m_hEff1D_xbb_xcc[tagType][0]->FindFixBin(jet.first);
  int iFlav = 0;
  if (jet_flav == 4) iFlav = 1;
