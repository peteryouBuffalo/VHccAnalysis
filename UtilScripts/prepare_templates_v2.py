...
for r in regions:
  ...
  hMC_total   = getHist(hN,samples_per_category["MC"],fHist,lumiScales)

  hN = r + "_unmatched_mass"
  hMC_unmatched = getHist(hN,samples_per_category["MC"],fHist,lumiScales)
  hN = r + "_matched_mass"
  hMC_matched   = getHist(hN,samples_per_category["MC"],fHist,lumiScales)
  ...
  for y in years:
    ...
    hMC_unmatched_in = hMC_unmatched[y]
    hMC_matched_in   = hMC_matched[y]

    # Rebin these plots to match the range and binning
    # that we want.
    hData_out         = Rebinning(hData_in, "data_obs_" + region_name[r] + "_nominal")
    hMC_unmatched_out = Rebinning(hMC_unmatched_in, "catp1_" + region_name[r] + "_nominal")
    hMC_matched_out   = Rebinning(hMC_matched_in, "catp2_" + region_name[r] + "_nominal")
    
    # Add these plots to the appropriate
    output_files[y].cd()
    hData_out.Write()
    hMC_unmatched_out.Write()
    hMC_matched_out.Write()
