...
def getHist(pN,samName,fH,lS): #samName = ['Electron'],['DY_0J','DY_1J','DY_2J']...
  ...
  #for y in ['17']:
    ...
    if samName[0] not in ['SingleMuon']:
    ...
    for iS in range(len(samName)):
      for fi in range(len(fH[samName[iS]][y])):
        ...
        if samName[iS] not in ['SingleMuon']:
...
#ss = ['SingleMuon','ZH_HToCC_ZToQQ','ggZH_HToCC_ZToQQ','ZH_HToBB_ZToQQ','ggZH_HToBB_ZToQQ','QCD_HT500to700','QCD_HT700to1000','QCD_HT1000to1500','QCD_HT1500to2000','QCD_HT2000toInf']
...
for s in ss:
  ...
  for y in years:
    ...
    fHist[s][y] = []
    for iN in names:
      #if 'SingleMuon' in iN: fNames[s][y].append(cfg.get('Paths','pathData') + '/' + iN)
      #else: fNames[s][y].append(cfg.get('Paths','pathMC') + '/' + iN)
      if 'SingleMuon' in iN: fNames[s][y].append(cfg.get('Paths','path') + '/' + iN)
    ...
    for iS in xSecTmps:
      filterEff = 1.
      if 'SingleMuon' not in s and correctFilterEff: filterEff = filter_effs[s]
...
for r in regions:
  ...
  for plN in plotNames:
    ...
    hDat = getHist(hN,['SingleMuon'],fHist,lumiScales)
    print(" ", hN)
    ...
    for y in years:
      ...
      if 'HMass' in plN:
        nums[r][y] = {}
        nums[r][y]['SingleMuon'] = getHistIntegral(hDat[y],lowM,highM)
      ...
      plotNames_process = []
      dataTitle = 'Data (SingleMuon, 20'+y+')' 
    ...
    if 'HMass' in plN:
        nums[r]['All'] = {}
        nums[r]['All']['SingleMuon'] = getHistIntegral(hDatA,lowM,highM)
...
if not breakVV:
  labels = ['QCD','WJ','ZJ','TT','ST','WW','VZ','ZHBB','WH_HToBB_WToQQ','Bkgr','ggZH_HToCC_ZToQQ','ZH_HToCC_ZToQQ','WH_HToCC_WToQQ','S/sqrt(B)','SingleMuon']
  label_translate = {'QCD':'QCD','WJ':'W+jets','ZJ':'Z+jets','TT':'t$\\bar{t}$','ST':'Single top','WW':'WW','VZ':'VZ','ZHBB':'ZH(H$\\rightarrow$bb)','WH_HToBB_WToQQ':'WH(H$\\rightarrow$bb)','Bkgr':'Total background','ggZH_HToCC_ZToQQ':'ggZH(H$\\rightarrow$cc)','ZH_HToCC_ZToQQ':'ZH(H$\\rightarrow$cc)','WH_HToCC_WToQQ':'WH(H$\\rightarrow$cc)','S/sqrt(B)':'S/$\sqrt{B}$','SingleMuon':'Data'}
else:
  labels = ['QCD','WJ','ZJ','TT','ST','VVcc','VVbb','VVother','WH_HToBB_WToQQ','Bkgr','ggZH_HToCC_ZToQQ','ZH_HToCC_ZToQQ','WH_HToCC_WToQQ','S/sqrt(B)','SingleMuon']
  label_translate = {'QCD':'QCD','WJ':'W+jets','ZJ':'Z+jets','TT':'t$\\bar{t}$','ST':'Single top','VVcc':'VZ (Z$\\rightarrow$cc)','VVbb':'VZ (Z$\\rightarrow$bb)','VVother':'VV (other)','ZHBB':'ZH(H$\\rightarrow$bb)','WH_HToBB_WToQQ':'WH(H$\\rightarrow$bb)','Bkgr':'Total background','ggZH_HToCC_ZToQQ':'ggZH(H$\\rightarrow$cc)','ZH_HToCC_ZToQQ':'ZH(H$\\rightarrow$cc)','WH_HToCC_WToQQ':'WH(H$\\rightarrow$cc)','S/sqrt(B)':'S/$\sqrt{B}$','SingleMuon':'Data'}
