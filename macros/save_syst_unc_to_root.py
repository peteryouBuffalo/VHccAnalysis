...
output_files = {}
for y in years:
  output_name = output_dir + "/up_dn_plots_20" + y + ".root"
  if debug:
    print(y, ": file = ", output_name)
  output_files[y] = ROOT.TFile.Open(output_name, "UPDATE")
    ...
    for v in variables:
      ...
      for s in systs:
        ...
        for y in years:
          ...
          plot_nom.GetXaxis().SetRangeUser(xA_range[0], xA_range[1])
          ...
          # Make the up & down plots
          plot_up.Divide(plot_nom)
          plot_up.SetLineColor(ROOT.kBlue)
          plot_dn.Divide(plot_nom)
          plot_dn.SetLineColor(ROOT.kRed)

          # Save the plots to the root file with an appropriate
          # name that includes 1) region, 2) variable name,
          # 3) systematic, 4) sample
          save_name = cat + "_" + r + "_" + v + "_" + s
          if r == '':
            save_name = cat + "_" + v + "_" + s
          if debug: print(">>>>>>>>> save_name = ", save_name)
          plot_up.SetName(save_name + "U")
          plot_dn.SetName(save_name + "D")

          # Make sure you add them to the appropriate file for the year
          output_files[y].cd()
          plot_up.Write()
          plot_dn.Write()
